...
RSpec.describe AppointmentNotification::Worker, type: :job do
  describe "#perform" do
    ...
    def mock_successful_delivery
      ...
      allow_any_instance_of(TwilioService).to receive(:response).and_return(response_double)
      ...
      allow_any_instance_of(TwilioService).to receive(:client).and_return(twilio_client)
    ...
    it "sends a whatsapp message when notification's next_communication_type is whatsapp" do
      ...
      expect_any_instance_of(TwilioService).to receive(:send_whatsapp)
      described_class.perform_async(notification.id)
      described_class.drain
    end
    ...
    it "does not create a TwilioService or Communication if notification has been previously attempted by all available methods" do
      create(:communication, :whatsapp, notification: notification)
      create(:communication, :sms, notification: notification)

      expect(Statsd.instance).to receive(:increment).with("appointment_notification.worker.skipped.no_next_communication_type")
      expect_any_instance_of(TwilioService).not_to receive(:send_whatsapp)
      expect {
        described_class.perform_async(notification.id)
        described_class.drain
      }.not_to change { Communication.count }
    ...
    describe "medication reminder experiment" do
      it "provides a sender if available" do
        ...
        expect(TwilioService).to receive(:new).with(sms_sender: /testnumber/).and_call_original
        expect_any_instance_of(TwilioService).to receive(:send_sms)
        described_class.perform_async(notification.id)
        described_class.drain
      end
      ...
      it "does not provide a sender if unavailable" do
        ...
        expect(TwilioService).to receive(:new).with(no_args).and_call_original
        expect_any_instance_of(TwilioService).to receive(:send_sms)
        described_class.perform_async(notification.id)
        described_class.drain
      end
